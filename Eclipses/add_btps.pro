...
;  rate_dressing[7,*] = [0.0, 0.0, 0.084, 0.027, 0.0]
  ...
  hot_companion_p = bkgnd[hotstars].companion.p
  cool_companion_p = bkgnd[coolstars].companion.p
...
;  secs = where(bkgnd.sec eq 1)
  ...
  for ii=0,mult-1 do begin
    ...
    nplanets = total(ceil( rate_fressin * nhotstars )) + $
                total(ceil( rate_dressing * ncoolstars ))
    ...
    print, "Created ", idx0, " Planets, expected ", nplanets
