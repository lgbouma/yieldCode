pro calc_optimal_npix_hack,ps=ps
  ...
  if (keyword_set(ps)) then begin
     ...
     set_plot,'ps'
     device,filen='optimal_npix_hack.eps',/encapsulated,xsize=6.5,ysize=9,/inches,$
  ...
  n = 1000
  r = 0.1 + 5.0*dindgen(n)/double(n) ; pixels
  npix = !PI*r^2.
...
; now step through apparent magnitude and optimize aperture size
  ...
  for i=0,n_mags-1 do begin
     ...
     for j=0,n-1 do begin
        calc_noise_hack, imag[i], 3600., noise, $
                    npix_aper = npix[j], frac_aper = frac[j]
        snr[j] = 1./noise
       ; print, i, j, imag[i], npix[j], snr[j]
     endfor
