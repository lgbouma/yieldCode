pro bin_observe, sstruct=sstruct, pstruct=pstruct, sfile=sfile, pfile=pfile, outfile=outfile, $
	...
	sys_limit=sys_limit, keep_ntra=keep_ntra, duty_cycle=duty_cycle, bk_file=bk_file, sp_file=sp_file, $
	ffi_len=ffi_len
...
;  obs = indgen(n_elements(star))
  ...
  if (keyword_set(al_aper)) then begin
	...
	;frac = frac_fits[star[obsid].dx, star[obsid].dy, fov_ind,star[obsid].npix-1]
        thisfrac = frac_fits[*,star[obsid].npix-1]
  ...
  if (total(planet[obs].dep) gt 0) then begin
    ...
    if (keyword_set(nodil)) then planet[obs].snr = planet[obs].dep_eff / noise $
    else planet[obs].snr = planet[obs].dep_eff / (dil*noise)
  endif else begin
    if (keyword_set(nodil)) then planet[obs].dep = SNR_MIN * noise $
      else planet[obs].dep = SNR_MIN * noise * dil
    planet[obs].r = star[obsid].r * sqrt(planet[obs].dep) / REARTH_IN_RSUN
    planet[obs].snr = SNR_MIN
  endelse

  ; Calculate SNR per transit
  exptime =  planet[obs].dur_eff * 24.0 * 3600.
  if (keyword_set(al_aper)) then begin
	frac=1.0
	npix_use=9.
  endif  else begin 
  ...
  if (total(planet.dep) gt 0) then begin
    if (keyword_set(nodil)) then planet[obs].snrtran =  planet[obs].dep_eff / noise $
    else planet[obs].snrtran =  planet[obs].dep_eff / (dil*noise)
    planet[obs].snrgress = planet[obs].snrtran * $
      ...
      if (keyword_set(nodil)) then planet[obs].snrtran = planet[obs].dep / noise $
         else planet[obs].snrtran = planet[obs].dep / (dil*noise)
    endelse
